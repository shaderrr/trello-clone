...
export default function Navbar({
  ...
  const { data: session } = useSession(); // Get the NextAuth session for Outlook
      ...
      <header className="border-b bg-white/80 backdrop-blur-sm sticky top-0 z-50">
        <div className="container mx-auto px-4 py-3 sm:py-4 flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2 sm:space-x-4">
            ...
            {session ? (
              <Button variant="outline" size="sm" onClick={() => signOut()}>Disconnect Calendar</Button>
            ) : (
              <Button variant="outline" size="sm" onClick={() => signIn("azure-ad")}>Connect Outlook</Button>
            )}
            <UserButton />
          </div>
        </div>
