            ...
            <DialogContent className="w-[95vw] max-w-[425px] mx-auto">
              ...
              <form className="space-y-4" onSubmit={onCreateTask}>
                ...
                <div className="space-y-2">
                  ...
                  <Input name="assignee" type="email" placeholder="Enter assignee's email" />
                </div>
                <div className="space-y-2">
...
export default function BoardPage() {
  ...
  async function handleUpdateTask(e: React.FormEvent<HTMLFormElement>) {
    ...
    const updates: Partial<Task> = {
      ...
      reminder: (formData.get("reminder") as "none" | "15 min" | "1 hour" | "3 hour") || "none",
    };

    // If reminder settings change, reset next_reminder_at to allow the backend to reschedule
  ...
  function handleDragOver(event: DragOverEvent) {
    ...
    const sourceColumn = columns.find((c) => c.tasks.some((t) => t.id === active.id));
    const targetColumn = columns.find((c) => c.id === over.id || c.tasks.some((t) => t.id === over.id));
  
    if (!sourceColumn || !targetColumn || sourceColumn.id !== targetColumn.id) return;
  
    const activeIndex = sourceColumn.tasks.findIndex((t) => t.id === active.id);
  ...
  async function handleDragEnd(event: DragEndEvent) {
    ...
    if (sourceColumn.id !== targetColumn.id || sourceColumn.tasks.findIndex(t => t.id === taskId) !== newIndex) {
        await moveTask(taskId, targetColumn.id, newIndex);

        if (task && task.reminder !== 'none') {
            let newNextReminderAt: string | null | undefined = undefined;

            if (["To Do", "In Progress"].includes(targetColumn.title) && !["To Do", "In Progress"].includes(sourceColumn.title)) {
                const nextTime = new Date();
                nextTime.setMinutes(nextTime.getMinutes() + 1);
                newNextReminderAt = nextTime.toISOString();
            } 
            else if (["Review", "Done"].includes(targetColumn.title) && !["Review", "Done"].includes(sourceColumn.title)) {
