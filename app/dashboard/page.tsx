...
// New component to reduce code duplication
const BoardCard = ({ board, viewMode }: { board: Board; viewMode: "grid" | "list" }) => {
  return (
...
export default function DashboardPage() {
  ...
  // Corrected logic for free users
  const canCreateBoard = !isFreeUser || boards.length === 0;

  // Use memoization for filtering logic
  const filteredBoards = useMemo(() => {
    // NOTE: Task count logic is currently non-functional as task counts are not fetched.
    // This part of the filter will not work as intended until that data is available.
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        ...
        <span className="text-lg text-gray-600">Loading your boards...</span>
      </div>
    );
  }
        ...
        <div className="mb-6 sm:mb-8">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 sm:mb-6 space-y-4 sm:space-y-0">
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center space-y-2 sm:space-y-0 sm:space-x-4">
              ...
              <Button onClick={handleCreateBoard} disabled={isCreatingBoard || (isFreeUser && boards.length >= 1)}>
                {isCreatingBoard ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : <Plus className="h-4 w-4 mr-2" />}
                Create Board
              </Button>
            </div>
          </div>
          ...
          {boards.length === 0 && !loading ? (
            <div className="text-center p-8 text-gray-500">
                <p>No boards found. Create your first one!</p>
            </div>
          ) : viewMode === "grid" ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6">
              {filteredBoards.map((board) => (
                <BoardCard key={board.id} board={board} viewMode="grid" />
              ))}
              <Card
            ...
            <div className="space-y-4">
              ...
              <Card
                ...
                onClick={handleCreateBoard}
              >
                <CardContent className="p-4 sm:p-6 flex flex-col sm:flex-row items-center justify-center sm:justify-start">
                  {isCreatingBoard ? (
                    <Loader2 className="h-6 w-6 sm:h-8 sm:w-8 animate-spin text-blue-400" />
                  ) : (
                    <Plus className="h-6 w-6 sm:h-8 sm:w-8 text-gray-400 group-hover:text-blue-600 mb-2 sm:mb-0 sm:mr-4" />
                  )}
                  <p className="text-sm sm:text-base text-gray-600 group-hover:text-blue-600 font-medium">
                    Create new board
                  </p>
      ...
      <Dialog open={isFilterOpen} onOpenChange={setIsFilterOpen}>
        <DialogContent className="w-[95vw] max-w-[425px] mx-auto">
          ...
          <div className="space-y-4">
            ...
            {/* Task Count filtering is here, but non-functional without task data */}
            <div className="space-y-2">
              <Label>Task Count</Label>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                <div>
                  <Label htmlFor="min-tasks" className="text-sm">Minimum</Label>
                  <Input
                    id="min-tasks"
                    type="number"
                    min="0"
                ...
                <div>
                  <Label htmlFor="max-tasks" className="text-sm">Maximum</Label>
                  <Input
                    id="max-tasks"
                    type="number"
                    min="0"
